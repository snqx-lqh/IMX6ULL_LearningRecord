...
{
    i2c_master_write_bytes(I2C1, MPU6050_ADDR, reg, &data, 1);
	return 0;
}

int mpu6050_read_one_byte(unsigned char reg,unsigned char *data)
{
    i2c_master_read_bytes(I2C1, MPU6050_ADDR, reg, data,1);
	return 0;
}

int mpu6050_read_bytes(unsigned char reg,unsigned char *data,unsigned char len)
{
	i2c_master_read_bytes(I2C1, MPU6050_ADDR, reg, data,len);
	return 0;
}
